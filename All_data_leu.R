...
list_io_d <- lapply(list_io, Divide, y = 1.54)
...
list_IG_d <- lapply(list_IG, Divide, y = 2.8)
...
list_IP_d <- lapply(list_IP, Divide, y = 1.6)
...
data_1510 <- subset(data, Peak >= 1343 & Peak <= 1345)
data_1458 <- subset(data, Peak >= 1140 & Peak <= 1142)
data_1285 <- subset(data, Peak >= 1458 & Peak <= 1460)
...
g + stat_compare_means(aes(x = as.factor(Peak), y = cma, group = UV), 
  ...
  labs(title = "Leucine surface boxplots", x = "Band (cm-1)", y = "Intensity (a.u.)") +
