setwd("C:/Users/mglar/Desktop/Doctorado/Raman/2024-04-05 His, Met, Leu Measurements/10%ww His")
library(ggplot2)
library(tidyverse)
library(dplyr)
...
ChangeNames <- function(x) {
  names(x) <- c("Peak", "Intensity", "UV", "Substrate")
  return(x)
}
...
list_io_d <- lapply(list_io, Divide, y = 7)
...
list_IG_d <- lapply(list_IG, Divide, y = 7)
...
list_IM_d <- lapply(list_IM, Divide, y = 14)
...
list_IP_d <- lapply(list_IP, Divide, y = 2.4)
...
data_1510 <- subset(data, Peak >= 1480 & Peak <= 1482)
data_1458 <- subset(data, Peak >= 1140 & Peak <= 1141)
data_1285 <- subset(data, Peak >= 1160 & Peak <= 1162)
...
g + stat_compare_means(aes(x = as.factor(Peak), y = cma, group = UV), 
  ...
  labs(title = "Histidine surface boxplots", x = "Band (cm-1)", y = "Intensity (a.u.)") +
