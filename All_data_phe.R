...
list_io_d <- lapply(list_io, Divide, y = 10)
...
list_IG_d <- lapply(list_IG, Divide, y = 5)
...
list_IM_d <- lapply(list_IM, Divide, y = 10)
...
list_IP_d <- lapply(list_IP, Divide, y = 5)
...
data_1510 <- subset(data, Peak >= 1003 & Peak <= 1004)
...
g + stat_compare_means(aes(x = as.factor(Peak), y = cma, group = UV), 
  ...
  theme(axis.text=element_text(size=26), axis.title=element_text(size=28),
        plot.title=element_text(size=36), legend.key.size = unit(1, 'cm'), 
        legend.text=element_text(size=22), legend.title=element_text(size=20)) +
  scale_y_continuous(trans='log10', limits = c(1,1000)) + 
  labs(title = "Phenylalanine surface boxplots", x = "Band (cm-1)", y = "Intensity (a.u.)") +
