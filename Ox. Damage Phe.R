setwd("C:/Users/mglar/Desktop/Doctorado/Raman/2024-03-12 Phe")
library(ggplot2)
library(tidyverse)
library(dplyr)
...
ChangeNames <- function(x) {
  names(x) <- c("Rep", "Peak", "Intensity", "Simulant")
  return(x)
}
Divide <- function(x) {
  x %>%
...
data_1510 <- subset(data_s, Peak >= 1586 & Peak <= 1587)
data_1458 <- subset(data_s, Peak >= 1606 & Peak <= 1607)
data_1285 <- subset(data_s, Peak >= 1207 & Peak <= 1209)
data_1198 <- subset(data_s, Peak >= 1031 & Peak <= 1032)
data_1154 <- subset(data_s, Peak >= 1004 & Peak <= 1005)
data_1000 <- subset(data_s, Peak >= 622 & Peak <= 623)
data_1001 <- subset(data_s, Peak >= 1324 & Peak <= 1326)
data_1002 <- subset(data_s, Peak >= 1150 & Peak <= 1151)
data_1003 <- subset(data_s, Peak >= 1410 & Peak <= 1411)



data_1510 <- subset(data_s, Peak >= 1003 & Peak <= 1004)
data_1458 <- subset(data_s, Peak >= 1140 & Peak <= 1141)
data_1285 <- subset(data_s, Peak >= 1613 & Peak <= 1615)
...
g + scale_y_continuous(trans='log10', limits = c(10,1000)) + 
  labs(title = "Band intensity differences between regolith simulants - Phenylalanine"
